...
15. 知识库正文占比按合格段落统计，不按复用词数或逐句覆盖率统计；硬事实仍单独执行逐句 100% 证据校验。
...
把生成正文中的段落或具体句子与知识片段绑定。普通参考素材可以绑定到段落；硬事实必须进一步绑定到具体句子。
...
- `ratio_basis`: 固定为 `supported_paragraphs`
...
```text
知识库支撑占比 = 合格支撑段落数 / 全部合格正文段落数
```

引言、正文段落和 FAQ 回答计入分母；H1/H2/H3 标题、单独的 Q 标签、Markdown 标记、图片标记、图片说明字符串和 URL 不计入。空段、纯列表标签和少于 5 个可见英文词的片段不构成独立段落。

一个段落至少存在一条有效的段落级 EvidenceLink 时计为支撑段落。该 EvidenceLink 必须指向当前项目中已发布、未失效的 KnowledgeChunk，并且检索/校验结果确认其能够支撑该段落中的至少一个实质性观点；仅出现一个无关公开链接不能计入。

该指标衡量“有多少段落使用了项目知识”，不衡量原文复制率，也不表示整段每句话均已被证实。产品规格、认证、尺寸、材料、产能、交期等硬事实继续采用句子级 EvidenceLink，并要求 `hard_fact_coverage_required = 1.0`。详细决定见 ADR-0006。
