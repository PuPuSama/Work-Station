...
- 状态：M2.0–M2.5 工程实现完成；真实浏览器隔离/验收与 M2.6 发布门待执行
- 确认日期：2026-08-17
- 启动授权日期：2026-08-20
...
- 在独立空 PostgreSQL 数据库执行 Alembic 至 `20260820_0032`，M2 专用 33 项 PostgreSQL 测试全部通过；MinIO 真实 put/get/list/签名/删除往返测试通过。
- 复用的旧开发数据库卷虽然标记为 `20260820_0030`，实际缺少 M2.1/M2.3 需要的约束和 `execution_idempotency_key`；未对该卷做破坏性修复，避免把 schema 漂移误报为代码验收通过。
- 真实浏览器页面可加载，但本地 OIDC 提供方不可用，登录入口停在登录页，故跨用户隔离和端到端导入流程仍待身份提供方恢复后验收。
