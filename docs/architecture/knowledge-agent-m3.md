...
| 2026-07-30 | BasicHybridRetriever | 完成（待完整回归） | 项目/模型隔离、RRF 双通道、元数据/产品过滤、可选 rerank 集成测试 |
| 2026-07-30 | PostgreSQL FTS GIN 索引 | 完成（待迁移验收） | Alembic `20260730_0004` |
| 待完成 | RetrievalPlan 与 SectionEvidencePack | 未开始 | 大纲版本、Scope 隔离、充分度、幂等持久化 |
| 待完成 | EvidenceLink 与覆盖率 | 未开始 | 段落比例、硬事实句子覆盖、失效重校验 |
| 待完成 | API 与只读证据视图 | 未开始 | 每个 Scope 显示状态、来源和排序解释 |
