...
> 历史 M7 迁移记录：下文固定的迁移 Head、切换能力、OIDC 身份映射和“当前尚不能上线”均属于当时背景，不是现行发布门禁。当前密码账号、配置及发布步骤见[README](../../README.md)和[配置指南](../configuration.md)。保留本文用于理解备份恢复、审计与授权设计，不据此直接执行旧迁移命令。
