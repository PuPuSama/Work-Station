...
class ServerDeliveryPackageTests(unittest.TestCase):
    ...
    def test_batch_skip_preserves_measurement_and_packages_without_second_check(self) -> None:
        from services.ai_rate_policy import apply_batch_humanization_skip
        for score in (None, 22.7, 35.6, 72):
            with self.subTest(score=score):
                task, objects = prepared()
                task.status = "draft_ready"
                task.initial_article = "# High AI test article\n\nOriginal draft."
                task.initial_article_hash = content_hash(task.initial_article)
                task.initial_ai_check = AICheck(score=score, article_hash=task.initial_article_hash)
                self.assertTrue(apply_batch_humanization_skip(task))
                ...
                with ZipFile(BytesIO(objects.archive)) as archive:
                    ...
                    metadata = json.loads(archive.read("metadata.json"))
                    self.assertEqual(metadata["ai_rate_percent"], score)
