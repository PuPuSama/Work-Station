...
def _from_row(row: Mapping[str, Any]) -> ImportProposal:
    return ImportProposal(
        ...
        execution_idempotency_key=(
            str(row["execution_idempotency_key"])
            if row["execution_idempotency_key"] is not None
            else None
        ),
...
def _values(proposal: ImportProposal) -> dict[str, object]:
    ...
    return {
        ...
        "execution_idempotency_key": proposal.execution_idempotency_key,
...
class PostgresImportProposalRepository:
    ...
    def _lock_attachment(
        ...
        *,
        ...
        statement = sa.select(assistant_attachments)
    ...
    def claim_execution(
        ...
        *,
        ...
        """Claim a confirmed proposal and move its attachment to importing.

        The execution key makes a worker crash replay-safe. A retry of the
        same durable execute Job may observe ``running`` or a terminal state
        and receives the existing state instead of creating another claim.
        """

        changed_at = _aware_utc(now, "now")
        key = str(execution_idempotency_key or "").strip()
        ...
        with self._engine.begin() as connection:
            ...
            if attachment is None:
                raise ImportProposalNotFound("attachment not found")
            stored_key = str((row.get("execution_idempotency_key") or "")).strip()
            if current.status in {"running", "waiting_publication", "completed"}:
                if stored_key != key:
                    raise ImportProposalConflict(
                        "proposal is already owned by another execution",
                        code="import_execution_conflict",
                        current_revision=current.revision,
                    )
                return ImportExecutionState(
                    proposal=current,
            ...
            if str(attachment["status"]) not in {"proposal_ready", "importing"}:
                raise ImportProposalConflict(
                    "attachment is not ready for import",
                    code="attachment_import_status_conflict",
                    current_revision=current.revision,
                )
            attachment_revision = int(attachment["revision"])
            if str(attachment["status"]) == "proposal_ready":
                attachment_row = connection.execute(
                    assistant_attachments.update()
                    .where(
                        assistant_attachments.c.organization_id == actor.organization_id,
                        assistant_attachments.c.creator_user_id == actor.user_id,
                        assistant_attachments.c.attachment_id == current.attachment_id,
                        assistant_attachments.c.revision == attachment_revision,
                        assistant_attachments.c.status == "proposal_ready",
                    )
                    .values(
                        status="importing",
                ...
                if attachment_row is None:
                    raise ImportProposalConflict(
                        "attachment import claim changed",
                        code="attachment_revision_conflict",
                    )
                attachment_revision = int(attachment_row["revision"])
            updated_row = connection.execute(
                ...
                .where(
                    *self._scope(actor, proposal_id),
                    ...
                    assistant_import_proposals.c.status == "confirmed",
                )
                .values(
                    status="running",
            ...
            running = _from_row(updated_row)
            self._audit.append(
                connection,
                self._audit_event(running, transition="running"),
            )
            return ImportExecutionState(
                proposal=running,
    ...
    def complete_execution(
        ...
        *,
        actor: ActorIdentity,
        proposal_id: str,
        expected_running_revision: int,
        execution_idempotency_key: str,
        resulting_entity_refs: tuple[Mapping[str, object], ...],
        waiting_publication: bool,
        now: datetime,
    ) -> ImportExecutionState:
        """Commit formal import refs and terminal Proposal/attachment state."""

        changed_at = _aware_utc(now, "now")
        key = str(execution_idempotency_key or "").strip()
        refs = _json_refs([dict(item) for item in resulting_entity_refs])
        final_status: ImportProposalStatus = (
            "waiting_publication" if waiting_publication else "completed"
        )
        with self._engine.begin() as connection:
            row = self._lock_for_actor(connection, actor, proposal_id)
            if row is None:
            ...
            if (
                ...
                or current.revision != expected_running_revision
                or stored_key != key
            ):
                raise ImportProposalConflict(
                    "import execution claim changed",
                    code="import_execution_conflict",
                    current_revision=current.revision,
                )
            updated_row = connection.execute(
                ...
                .where(
                    *self._scope(actor, proposal_id),
                    assistant_import_proposals.c.revision == expected_running_revision,
                    assistant_import_proposals.c.status == "running",
                )
                .values(
                    status=final_status,
                    revision=expected_running_revision + 1,
                    resulting_entity_refs=[dict(item) for item in refs],
            ...
            attachment_revision = int(attachment["revision"])
            if attachment_status == "importing":
                attachment_row = connection.execute(
                    assistant_attachments.update()
                    .where(
                        assistant_attachments.c.organization_id == actor.organization_id,
                        ...
                        assistant_attachments.c.attachment_id == current.attachment_id,
                        assistant_attachments.c.revision == attachment_revision,
                        assistant_attachments.c.status == "importing",
                    )
                    .values(
                        status="imported",
                        revision=assistant_attachments.c.revision + 1,
                ...
                if attachment_row is None:
                    raise ImportProposalConflict(
                        "attachment completion claim changed",
                        code="attachment_revision_conflict",
                    )
                attachment_revision = int(attachment_row["revision"])
            finished = _from_row(updated_row)
            self._audit.append(
                connection,
                self._audit_event(finished, transition=final_status),
            )
            return ImportExecutionState(
                proposal=finished,
                attachment_revision=attachment_revision,
    ...
    def fail_execution(
        ...
        *,
        ...
        """Move a non-retryable execution into an auditable failed state."""

        changed_at = _aware_utc(now, "now")
        normalized_error = str(error_code or "").strip()[:255] or "import_failed"
        with self._engine.begin() as connection:
            row = self._lock_for_actor(connection, actor, proposal_id)
            if row is None:
                raise ImportProposalNotFound("import proposal not found")
            ...
            if current.status == "failed":
                return ImportExecutionState(
                    ...
                    attachment_revision=int(
                        connection.execute(
                            sa.select(assistant_attachments.c.revision).where(
                                assistant_attachments.c.organization_id == actor.organization_id,
                                assistant_attachments.c.attachment_id == current.attachment_id,
                                assistant_attachments.c.creator_user_id == actor.user_id,
                            )
            ...
            if (
                current.status != "running"
                or current.revision != expected_running_revision
                or str(row.get("execution_idempotency_key") or "").strip()
                != execution_idempotency_key
            ):
                raise ImportProposalConflict(
                    "import execution claim changed",
            ...
            updated = connection.execute(
                ...
                .values(
                    status="failed",
                    revision=expected_running_revision + 1,
                    standardized_error_code=normalized_error,
                    updated_at=changed_at,
                )
                .returning(*assistant_import_proposals.c)
            ).mappings().one_or_none()
            ...
            if attachment is not None:
                ...
                if attachment["status"] == "importing":
                    reverted = connection.execute(
                        ...
                        .returning(assistant_attachments.c.revision)
                    ).scalar_one_or_none()
                    if reverted is not None:
                        attachment_revision = int(reverted)
            failed = _from_row(updated)
            self._audit.append(
                connection,
                self._audit_event(failed, transition="failed"),
            )
            return ImportExecutionState(
                proposal=failed,
                attachment_revision=attachment_revision,
