...
class ServerInitialAiScreenshotPreparation:
    """Normalize and store one initial-review screenshot without paths."""
        ...
        *,
        ...
        if not task.initial_article.strip():
            raise ServerAiScreenshotError(
                "the initial article must be saved first"
            )
        body = bytes(content)
        ...
        asset = self._objects.upload_initial_ai_screenshot(
            actor=actor,
        ...
        check.screenshot_filename = "initial-ai-rate.png"
