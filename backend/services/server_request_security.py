...
def server_http_route_available(method: str, path: str) -> bool:
    ...
    if (
        ...
        and parts[1:3] == ["api", "projects"]
        and bool(parts[3])
        and parts[4:6] == ["metadata", "business-profile-draft"]
    ):
        return True
    if (
    ...
    if (
        ...
        and bool(parts[5])
        and parts[6] == "checks"
        and parts[7] in {"final-ai", "initial-ai", "knowledge-coverage"}
    ):
        return True
    if (
        normalized_method == "GET"
        ...
        and parts[4] == "tasks"
        and bool(parts[5])
        and parts[6:8] == ["checks", "knowledge-coverage"]
    ):
        return True
    if (
        normalized_method == "GET"
    ...
    if (
        ...
        and parts[1:3] == ["api", "projects"]
        and bool(parts[3])
        and parts[4:6] == ["wordpress", "test-connection"]
    ):
        return True
    if (
        normalized_method in {"GET", "PUT"}
        ...
        and parts[1:3] == ["api", "projects"]
        and bool(parts[3])
        and parts[4:6] == ["wordpress", "credentials"]
    ):
        return True
    if (
        normalized_method == "POST"
